...
% x'' + 4.26 * x' + 507.35 * x = 10 * sin(omega_n * t)
equacao_movimento_x = @(t, x) [x(2); -4.26 * x(2) - 507.35 * x(1) + 10 * sin(omega_n * t)];
...
xlabel('Tempo (s)');
ylabel('Posição (m)');
title('Resposta Temporal de X(t) na Frequência de Ressonância');
grid on;
...
xlabel('Tempo (s)');
ylabel('Ângulo (graus)');
title('Resposta Temporal de Theta(t) na Frequência de Ressonância');
grid on;
